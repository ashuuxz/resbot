                ...
                if (plugin.limitDeduction && (!isOwner(sender) || !isPremiumUser(sender))) {
                    try {
                        ...
                        if (dataUsers) {
                            ...
                            const newLimit = dataUsers.limit - plugin.limitDeduction;
                            await updateUser(sender, { limit: newLimit });
                        } else {
                        }
                    } catch (error) {
...
async function participantUpdate(sock, messageInfo) {
    ...
    try {
        ...
        if (settingGroups) {
            ...
            //console.log(chalk.yellowBright(`Grup tidak ditemukan: ${id}`));
        }
    } catch (error) {
