...
const { getCache, setCache } = require('@lib/globalCache');
...
async function process(sock, messageInfo) {
    ...
    let dataSewa;
    const cachedData = getCache(`sewa-${remoteJid}`);
    if (cachedData) {
        dataSewa = cachedData.data; // Menggunakan data dari cache
    } else {
        dataSewa = await findSewa(remoteJid); // Mengambil data baru
        setCache(`sewa-${remoteJid}`, dataSewa);
    }
